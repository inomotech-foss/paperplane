...
| redis.appendonly        |               true                |          | Enables the Redis append-only log. Without it, keys written since the last snapshot are lost on restart, which drops OAuth tokens and makes every MCP client authorize again. |
...
| minio.image                  | bitnamilegacy/minio:2025.7.23-debian-12-r5 |          | Using this key, user must provide the docker image name to setup the stateful deployment of `minio`. Its entrypoint creates the `env.docstore_bucket` bucket with anonymous download access on start-up. (must be set when `minio.local_setup=true`) |
...
The bundled MinIO image creates the `env.docstore_bucket` bucket with anonymous
download access on start-up, so the chart no longer runs a separate `mc` job and
the former `minio.image_mc` value is ignored. The pod starts as root only so the
entrypoint can fix the ownership of volumes written by the old `minio/minio`
image; the server itself runs as the unprivileged `minio` user.
...
| api.memoryRequest      |                   512Mi                    |          | Every deployment in kubernetes can be set to use minimum memory they are allowed to use. This key sets the memory request for this deployment to use.                                                           |
...
| worker.concurrency   |                     4                      |          | Number of celery child processes. Left unset, celery forks one per node core regardless of the CPU limit.                                                                    |
| worker.memoryLimit   |                   2000Mi                   |          | Every deployment in kubernetes can be set to use maximum memory they are allowed to use. This key sets the memory limit for this deployment to use.                                                             |
...
| worker.memoryRequest |                   512Mi                    |          | Every deployment in kubernetes can be set to use minimum memory they are allowed to use. This key sets the memory request for this deployment to use.                                                           |
...
| beatworker.memoryRequest |                   256Mi                    |          | Every deployment in kubernetes can be set to use minimum memory they are allowed to use. This key sets the memory request for this deployment to use.                                                           |
