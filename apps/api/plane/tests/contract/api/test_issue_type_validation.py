...
def state_b(db, workspace, project_b):
    return State.objects.create(name="Todo", group="backlog", project=project_b, workspace=workspace, default=True)
...
class TestWorkItemTypeCrossProjectValidation:
    @pytest.mark.django_db
    def test_create_work_item_with_foreign_project_type_rejected(
        self, api_key_client, workspace, project_b, state_b, type_a
    ):
        response = api_key_client.post(
            work_items_url(workspace.slug, project_b.id),
